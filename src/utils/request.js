...
function request(options) {
	const { method = "post", url: urlParam, data, headers, isNeedErrMsg = true, ...restOptions } = options;
	...
	return new Promise((resolve, reject) => {
		axios
			.request({
				method,
				url: url.indexOf("http:") > -1 || url.indexOf("https:") > -1 ? url : ENV_CONFIG.apiPath + url,
				data: postData,
				headers: {
